// One-off: dump display/grid-template of a selector and its children.
...
const r = await send('Runtime.evaluate', {
  expression: `(()=>{var e=document.querySelector(${JSON.stringify(sel)});if(!e)return 'NOT FOUND';
    ...
    var kids=[...e.children].map(k=>{var kc=getComputedStyle(k),b=k.getBoundingClientRect();
      return {tag:k.tagName,cls:k.className,disp:kc.display,col:kc.gridColumn,x:Math.round(b.left),w:Math.round(b.width)};});
    return JSON.stringify({display:c.display,gtc:c.gridTemplateColumns,gaf:c.gridAutoFlow,gap:c.gap,kids},null,1);})()`,
  returnByValue: true,
});
