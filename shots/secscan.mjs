// Per-top-level-block: bounding box and scrollWidth, to localise a sideways scroll.
...
await sleep(2200);
// Hide one top-level block at a time; report which one drops the scroll width.
const r = await send('Runtime.evaluate', {
  expression: `(()=>{var de=document.documentElement,base=de.scrollWidth,out=[];
    var blocks=[].concat([].slice.call(document.body.children));
    blocks.forEach(function(b){
      var main=b.tagName==='MAIN'?[].slice.call(b.children):[b];
      main.forEach(function(e){
        var d=e.style.display; e.style.display='none';
        var w=de.scrollWidth; e.style.display=d;
        if(w<base) out.push({sel:e.tagName.toLowerCase()+(typeof e.className==='string'&&e.className?'.'+e.className.trim().split(/\s+/).join('.'):''),drops:base-w});
      });
    });
    return JSON.stringify({clientW:de.clientWidth,scrollW:base,culprits:out},null,1);})()`,
  returnByValue: true,
});
