// Bounding boxes for a selector and its first child, at a given width.
...
const r = await send('Runtime.evaluate', {
  expression: `(()=>{var out=[];document.querySelectorAll(${JSON.stringify(sel)}).forEach(function(e,i){
    if(i>3)return;var b=e.getBoundingClientRect(),c=getComputedStyle(e);
    out.push({sel:${JSON.stringify(sel)},i:i,w:Math.round(b.width),h:Math.round(b.height),
      disp:c.display,ar:c.aspectRatio,ht:c.height,pos:c.position});});
    return JSON.stringify(out,null,1);})()`,
  returnByValue: true,
});
console.log(width, r.result?.result?.value);
