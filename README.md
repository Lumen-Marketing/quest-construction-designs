...
directions are held to a fifty-four page contract. The service and area breadcrumbs point at them
...
zero-dependency Node generator walks ten direction modules over the fifty-four page manifest and
...
holds all of them to the same guarantees — fifty-four pages, one `h1` each, alt text and intrinsic
...
- A JSON-LD `@graph`: `GeneralContractor` + `HomeAndConstructionBusiness` (real telephone,
  founding year, `areaServed` for all thirty-four cities, `knowsAbout` the fourteen trades), `WebSite`,
...
- **No invented figures anywhere.** Project counts, years-in-business, percentages and review
  ...
  fourteen trades, thirty-four cities, founded 2018, reachable 24/7. A test in `all.test.mjs` fails the
